...
func (s *Store) Match(revision, key, mediaType string, id int) (domain.Snapshot, error) {
	if (mediaType != "tm" && mediaType != "tv") || id <= 0 {
		return domain.Snapshot{}, fmt.Errorf("invalid media reference")
	}
	return s.mutate(revision, func(lines []line, snapshot domain.Snapshot) ([]line, error) {
		lineNumber, ok := findLine(snapshot, key)
		if !ok {
...
func (s *Store) BatchMatch(revision string, matches []MatchInput) (domain.Snapshot, error) {
	for _, match := range matches {
		if (match.Type != "tm" && match.Type != "tv") || match.ID <= 0 {
			return domain.Snapshot{}, fmt.Errorf("invalid media reference")
		}
	}
	return s.mutate(revision, func(lines []line, snapshot domain.Snapshot) ([]line, error) {
		records := make(map[string]domain.Record, len(snapshot.Records))
		for _, record := range snapshot.Records {
			records[record.Key] = record
		}
		for _, match := range matches {
			target, ok := records[match.Key]
			if !ok {
				return nil, ErrNotFound
			}
			input := parser.InputFromRecord(target)
			input.MediaRef = &domain.MediaRef{Type: match.Type, ID: match.ID}
			serialized, err := parser.Serialize(input)
			if err != nil {
				return nil, err
			}
			lines[target.LineNumber-1].content = serialized
		}
		return lines, nil
	})
}
...
func (s *Store) mutate(expected string, mutate func([]line, domain.Snapshot) ([]line, error)) (domain.Snapshot, error) {
	...
	if err := s.writeAtomic([]byte(joinLines(lines)), data); err != nil {
		return domain.Snapshot{}, err
	}
	return s.readUnlocked()
}
...
func (s *Store) writeAtomic(next, previous []byte) error {
	...
	if err := writeSynced(filepath.Join(backupDir, backupName), previous); err != nil {
		return err
	}
...
func splitLines(value string) []line {
	...
	for len(value) > 0 {
		...
		if index < 0 {
			result = append(result, line{content: value})
			break
		}
		content, ending := value[:index], "\n"
		if strings.HasSuffix(content, "\r") {
			content, ending = strings.TrimSuffix(content, "\r"), "\r\n"
		}
...
func joinLines(lines []line) string {
	...
	for _, item := range lines {
		...
		b.WriteString(item.ending)
	}
	return b.String()
}
func dominantEnding(lines []line) string {
	crlf, lf := 0, 0
	for _, item := range lines {
		if item.ending == "\r\n" {
			crlf++
...
func keyFor(revision, lineHash string, occurrence int) string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%s:%s:%d", revision, lineHash, occurrence)))
	return hex.EncodeToString(sum[:12])
}
func findLine(snapshot domain.Snapshot, key string) (int, bool) {
	for _, record := range snapshot.Records {
		if record.Key == key {
