...
包含 `tm:` ID 的电影会在详情页显示“播放”按钮。后端按配置顺序查询，命中第一条结果后停止；前端使用 ArtPlayer 与 Mediabunny Proxy 在站内播放 Emby 返回的静态视频地址，包括 MKV 容器。剧集只有 TMDB 剧集 ID、没有具体集 ID，因此暂不显示播放入口。

视频由浏览器直接向最终媒体地址读取，Traker 后端不代理视频数据。跨域媒体服务需要允许 Traker 页面来源访问，并支持 Range 请求；实际能否播放还取决于浏览器的 WebCodecs 编解码器支持。读取或解码失败时，错误会显示在播放器下方。
